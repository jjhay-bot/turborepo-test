...
type Screen = "discover" | "results" | "profile" | "chat" | "complete" | "rating" | "success" | "merchantHome" | "merchantSetup" | "merchantListings" | "merchantInbox" | "merchantChat";
...
export default function HanapDitoPrototypePage() {
  ...
  const [businessName, setBusinessName] = useState("Kuya Ben Home Repair");
  const [serviceName, setServiceName] = useState("Faucet repair");
  ...
  const reset = () => {
    ...
    setRatingTags(["Fast response", "Accurate listing"]); setRole("customer");
  };
  const switchRole = () => {
    const next = role === "customer" ? "merchant" : "customer";
    setRole(next); setScreen(next === "merchant" ? "merchantHome" : "discover");
  };
  ...
  return (
    <Box sx={{ minHeight: "100dvh", bgcolor: "#e9efe9", py: { xs: 0, sm: 3 } }}>
      <Paper elevation={6} sx={{ width: "100%", maxWidth: 430, minHeight: { xs: "100dvh", sm: 820 }, mx: "auto", overflow: "hidden", position: "relative", borderRadius: { xs: 0, sm: 5 }, bgcolor: "#f7f9f6" }}>
        ...
        {screen === "merchantHome" && <Box>{header()}<Stack spacing={2} sx={{ p: 2.2 }}><Box><Typography variant="overline" sx={{ color: green, fontWeight: 800 }}>MERCHANT MODE</Typography><Typography variant="h4" fontWeight={900}>Good morning, Kuya Ben</Typography><Typography color="text.secondary">Manage your visibility and nearby inquiries.</Typography></Box><Paper variant="outlined" sx={{ p: 1.7, borderRadius: 3, bgcolor: available ? soft : "white" }}><Stack direction="row" justifyContent="space-between" alignItems="center"><Box><Typography fontWeight={900}>{available ? "Available today" : "Not available today"}</Typography><Typography variant="caption" color="text.secondary">Customers can see this status immediately.</Typography></Box><Button variant={available ? "contained" : "outlined"} onClick={() => setAvailable(!available)}>{available ? "Turn off" : "Go online"}</Button></Stack></Paper><Stack direction="row" spacing={1}><Paper variant="outlined" sx={{ p: 1.5, flex: 1, borderRadius: 3 }}><Typography variant="h5" fontWeight={900}>3</Typography><Typography variant="caption">New inquiries</Typography></Paper><Paper variant="outlined" sx={{ p: 1.5, flex: 1, borderRadius: 3 }}><Typography variant="h5" fontWeight={900}>23</Typography><Typography variant="caption">Confirmed jobs</Typography></Paper><Paper variant="outlined" sx={{ p: 1.5, flex: 1, borderRadius: 3 }}><Typography variant="h5" fontWeight={900}>4.8</Typography><Typography variant="caption">Rating</Typography></Paper></Stack><Typography fontWeight={900}>Merchant workspace</Typography><Button size="large" variant="contained" onClick={() => setScreen("merchantInbox")}>Open inquiry inbox · 3 new</Button><Button size="large" variant="outlined" onClick={() => setScreen("merchantListings")}>Manage products & services</Button><Button size="large" variant="outlined" onClick={() => setScreen("merchantSetup")}>Edit store profile</Button><Paper variant="outlined" sx={{ p: 1.5, borderRadius: 3 }}><Typography fontWeight={850}>Public profile status</Typography><Typography variant="body2" color="text.secondary">Approved · 3 active services · Abar 2nd · 8 km service radius</Typography></Paper></Stack></Box>}

        {screen === "merchantSetup" && <Box>{header("Store profile", "merchantHome")}<Stack spacing={1.6} sx={{ p: 2 }}><Typography variant="overline" sx={{ color: green, fontWeight: 800 }}>ONBOARDING / EDITOR</Typography><Typography variant="h5" fontWeight={900}>How should customers find you?</Typography><TextField label="Business or seller name" value={businessName} onChange={(e) => setBusinessName(e.target.value)} /><TextField label="Short description" multiline minRows={3} defaultValue="Home repair and plumbing help around San Jose City. Available for same-day visits." /><TextField label="Barangay / general location" defaultValue="Abar 2nd, San Jose City" /><TextField label="Service radius" defaultValue="8 km" /><TextField label="Phone number" defaultValue="09•• ••• ••42" /><TextField label="Messenger link (optional)" defaultValue="m.me/kuyabenrepair" /><Typography variant="caption" color="text.secondary">Only your barangay/general area is public. Exact location stays private.</Typography><Button size="large" variant="contained" onClick={() => { window.alert("Store profile saved."); setScreen("merchantHome"); }}>Save profile</Button></Stack></Box>}

        {screen === "merchantListings" && <Box>{header("Products & services", "merchantHome")}<Stack spacing={1.5} sx={{ p: 2 }}><Typography variant="overline" sx={{ color: green, fontWeight: 800 }}>3 ACTIVE LISTINGS</Typography><Paper variant="outlined" sx={{ p: 1.5, borderRadius: 3 }}><Typography fontWeight={900}>Faucet repair</Typography><Typography variant="body2" color="text.secondary">From ₱250 · Home repair · Active</Typography></Paper><Paper variant="outlined" sx={{ p: 1.5, borderRadius: 3 }}><Typography fontWeight={900}>Leak inspection</Typography><Typography variant="body2" color="text.secondary">Ask price · Plumbing · Active</Typography></Paper><Paper variant="outlined" sx={{ p: 1.5, borderRadius: 3 }}><Typography fontWeight={900}>Basic electrical repair</Typography><Typography variant="body2" color="text.secondary">From ₱300 · Home repair · Active</Typography></Paper><Typography variant="h6" fontWeight={900}>Add or edit a service</Typography><TextField label="Product or service name" value={serviceName} onChange={(e) => setServiceName(e.target.value)} /><TextField label="Starting price (PHP)" value={servicePrice} onChange={(e) => setServicePrice(e.target.value)} /><TextField label="Category / search tags" defaultValue="Home repair, plumber, faucet" /><Button variant="contained" onClick={() => window.alert("Listing saved for prototype.")}>Save listing</Button></Stack></Box>}

        {screen === "merchantInbox" && <Box>{header("Inquiry inbox", "merchantHome")}<Stack spacing={1.3} sx={{ p: 2 }}><Stack direction="row" spacing={1}><Chip label="All" sx={{ bgcolor: ink, color: "white" }} /><Chip label="New · 3" /><Chip label="In progress" /></Stack><Paper variant="outlined" onClick={() => setScreen("merchantChat")} sx={{ p: 1.5, borderRadius: 3, cursor: "pointer", borderColor: green }}><Stack direction="row" justifyContent="space-between"><Typography fontWeight={900}>Juan D. · Faucet repair</Typography><Chip size="small" label="NEW" sx={{ bgcolor: soft, color: green }} /></Stack><Typography variant="body2" color="text.secondary">Abar 1st · 1.3 km away</Typography><Typography sx={{ mt: 1 }}>“Available kayo today? Tumutulo po faucet namin.”</Typography><Typography variant="caption" color="text.secondary">2 minutes ago</Typography></Paper><Paper variant="outlined" sx={{ p: 1.5, borderRadius: 3 }}><Typography fontWeight={900}>Maria S. · Leak inspection</Typography><Typography variant="body2" color="text.secondary">Caanawan · In progress · 18 min ago</Typography></Paper><Paper variant="outlined" sx={{ p: 1.5, borderRadius: 3 }}><Typography fontWeight={900}>Paolo R. · Electrical repair</Typography><Typography variant="body2" color="text.secondary">Abar 2nd · Waiting for customer · Yesterday</Typography></Paper></Stack></Box>}

        {screen === "merchantChat" && <Box>{header("Juan D. · Faucet repair", "merchantInbox")}<Box sx={{ p: 1.5, bgcolor: "#fff8d9", borderBottom: "1px solid #ead888" }}><Typography fontWeight={800}>New inquiry · Abar 1st</Typography><Typography variant="caption">Customer is about 1.3 km away</Typography></Box><Stack spacing={1} sx={{ p: 1.5, pb: 19 }}>{messages.map((message) => <Box key={message.id} sx={{ alignSelf: message.from === "merchant" ? "flex-end" : "flex-start", maxWidth: "82%", bgcolor: message.from === "merchant" ? green : "white", color: message.from === "merchant" ? "white" : ink, border: "1px solid #dfe6e0", borderRadius: 3, px: 1.5, py: 1 }}>{message.text}</Box>)}</Stack><Paper square sx={{ position: "absolute", bottom: 0, left: 0, right: 0, p: 1.2 }}><Stack direction="row" spacing={1}><TextField fullWidth size="small" value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Reply to customer…" /><IconButton color="primary" onClick={() => { if (!draft.trim()) return; setMessages((current) => [...current, { id: Date.now(), from: "merchant", text: draft.trim() }]); setDraft(""); }}><SendRounded /></IconButton></Stack><Stack direction="row" spacing={1} sx={{ mt: 1 }}><Button fullWidth variant="outlined" onClick={() => window.alert("Inquiry declined; reason recorded.")}>Decline</Button><Button fullWidth variant="contained" onClick={() => window.alert("Marked in progress. Customer is notified.")}>Accept inquiry</Button></Stack><Button fullWidth color="success" sx={{ mt: 1 }} onClick={() => { window.alert("Merchant marked this completed. Waiting for customer confirmation."); setScreen("merchantHome"); }}>Mark service completed</Button></Paper></Box>}
